--[[	Continuity.lua (MPDX) by Ku-rin
	
]]

function initContinuity(restoring_game)

	Continuity.state = nil

	if restoring_game then

		if Game.restore_saved() then
		
			Continuity.state = "restored"

		end
	
	else

		if Game.restore_passed() then
		
			Continuity.state = "passed"
	...
	if Continuity.state then
	
		Continuity.unstashAll()
...
Continuity.stashAll = function()
	Players[0].overlays[1].text = "Stashing Begin"
	--Stash.doors()
...
Continuity.unstashAll = function()
	Players[0].overlays[2].text = "Unstashing Begin"
	--Unstash.doors()
	
	for k, v in pairs(Unstash) do
		v()
	end
