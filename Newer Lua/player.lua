...
function playerIdleUpkeep()
	...
	Player.me.overlays[0].text = Continuity.state
...
Player.actions.defaultInteraction = function()

	local target, x, y, z, polygon = Player.me:find_target()

	Players.print(target)

	local distance = getSlantDistance(x, y, z, Player.me)

	if distance <= 1 then
	
		if is_scenery(target) then
		
			if target._owner then
				target._owner:interaction()
			end
		...
		elseif is_polygon_floor(target) or is_polygon_ceiling(target) or is_side(target) then
		
			if Surfaces.evaluate(target, x, y, z, polygon) == false then
				Players.print("No Surface Features.")
			else
				return
			end
				
			if not Kinetics.evaluate(target, x, y, z, polygon) == false then
				Players.print("No Motions.")
			else
				return
			end
			
		elseif is_polygon(target) then
...
Player.actions.examineSide = function()
	
	local target, x, y, z, polygon = Player.me:find_target()
	
	Players.print("Examining...")
	
	if is_side(target) then
		Players.print("Texture X Primary: " .. target.primary.texture_x)
		Players.print("Texture Y Primary: " .. target.primary.texture_y)
		Players.print("Texture X Secondary: " .. target.secondary.texture_x)
		Players.print("Texture Y Secondary: " .. target.secondary.texture_y)
		Players.print("Texture X Transparent: " .. target.transparent.texture_x)
		Players.print("Texture Y Transparent: " .. target.transparent.texture_y)
		
		local lprev1 = target.primary.light
		local lprev2 = target.secondary.light
		local lprev3 = target.transparent.light
		target.primary.light = Lights[20]
		target.secondary.light = Lights[20]
		target.transparent.light = Lights[20]
		
		local fixlite = function()
			target.primary.light = lprev1
			target.secondary.light = lprev2
			target.transparent.light = lprev3
		end
		
		createTimer(5, false, fixlite)
